...
void app_main(void)
{
    ...
    while (true){
        ...
        if (app_event & APP_EVENT_WIFI_START){
        ...
        if (app_event & APP_EVENT_BLE_NOTIFY_WIFI_SCAN_DONE){
        ...
        if (app_event & APP_EVENT_BLE_RECEIVE_WIFI_CREDS_DONE){
        ...
        if (app_event & APP_EVENT_BLE_NOTIFY_WIFI_CREDS_DONE){
            ESP_LOGI(TAG, "APP_EVENT_BLE_NOTIFY_WIFI_CREDS_DONE");
            if (app_state == APP_STATE_AWAITING_BLE_NOTIFY_WIFI_CREDS_DONE){
                app_state = APP_STATE_NORMAL_OPERATION;
            }
        }
        if (app_event & APP_EVENT_WIFI_CONNECTED){
        ...
        if (app_event & APP_EVENT_WIFI_DISCONNECTED){
            if (app_state == APP_STATE_AWAITING_BLE_NOTIFY_WIFI_CREDS_DONE){
                app_state = APP_STATE_AWAITING_BLE_NOTIFY_WIFI_CREDS_DONE;
        ...
        if (app_event & APP_EVENT_BLE_GAP_CONNECT){
        ...
        if (app_event & APP_EVENT_BLE_GAP_DISCONNECT){
